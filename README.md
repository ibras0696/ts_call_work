...
## 11. Roadmap (что легко добавить)
- Настоящая транскрипция (ASR) и индексация по тексту.
- Страница/эндпойнт прогресса обработки.
- Выдача деталей записи в GET /calls/{id} (duration, transcription, silence_marks).
- Тесты (pytest) и линтинг (ruff, mypy) в CI.
- Хук выгрузки файлов в S3 после аплоада.


## 12. Структура репозитория (основное)
```
.
├── app/
│   ├── api/
...
Сделано с заботой о читаемости кода, надёжности и простоте локального запуска. Если хотите — добавлю тесты, CI и расширенный API под ваши сценарии.
